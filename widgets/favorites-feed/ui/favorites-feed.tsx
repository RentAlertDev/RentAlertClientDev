...
import { FavoriteCard, FAVORITES_PAGE_SIZE, FavoritesEmptyState, useFavoritesQuery, useRemoveFavoriteMutation } from '@/modules/favorite'
...
export function FavoritesFeed() {
	...
	const confirmRemove = () => {
		...
		removeMutation.mutate(removingId, { onSuccess: () => { setRemovingId(null); toast.success('Объявление удалено из избранного') }, onError: () => toast.error('Не удалось удалить из избранного') })
	}
	...
	return <main className='min-h-dvh bg-[var(--background)] px-4 py-5 text-[var(--foreground)] sm:px-6'>
		<div className='mx-auto flex w-full max-w-3xl flex-col gap-5 pb-6'>
			...
			{isLoading ? <div className='grid min-h-64 place-items-center'><Loader label='Загружаем избранное' /></div> : !query.isError && query.data?.content.length === 0 ? <FavoritesEmptyState /> : <div className='grid gap-4'>{query.data?.content.map(favorite => <FavoriteCard favorite={favorite} isRemoving={removeMutation.isPending && removingId === favorite.listing.id} key={favorite.listing.id} onRemove={() => setRemovingId(favorite.listing.id)} />)}</div>}
