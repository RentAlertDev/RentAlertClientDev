	...
	return (
		<article className='flex gap-3 rounded-lg border border-[var(--card-border)] bg-[var(--card)] p-3 shadow-[0_8px_24px_var(--card-shadow)]'>
			...
			<div className='min-w-0 flex-1'>
				...
				<div className='mt-2 flex items-center gap-1.5 text-xs font-medium text-[var(--primary)]'><Clock3 aria-hidden className='size-3.5' />{new Intl.DateTimeFormat('ru-RU', { hour: '2-digit', minute: '2-digit' }).format(date)}</div>
